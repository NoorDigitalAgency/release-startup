...
async function run(): Promise<void> {
  ...
  try {
    ...
    const repositoryUrl = context.payload.repository!.clone_url!.replace(/(https:\/\/)(.+)/g, (_: string, _1: string, _2: string) => `${_1}${context.actor}:${token}@${_2}`);
      ...
      if (!hotfix) {
        ...
        if ((stage === 'beta' || stage === 'production')) {
          ...
          if (scriptFileWithShebang) {
            ...
            const {stderr, exitCode, stdout: zxStdout} = await shell(scriptFile, args);
